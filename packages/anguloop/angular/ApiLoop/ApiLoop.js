app.provider('ApiLoop', function() {
  ...
  this.enableSchemaFetcher = function() {
    // fetch schema on Collection.fetch()
    ApiLoopProvider.config.events['collection:beforeFetch'].push(function() {
      var self = this;
      // loopback doesnt expose _meta endpoint in nested ressources
      return this._config.connector.get(ApiLoopProvider.config.urlPrefix + '/' + this.name + '/_meta').then(function(res) {
  ...
  this.enableModelUpdater = function() {
    // apply model-updater
