function parseNumber(val) {
  // workaround ui-router $stateParams type inconsistency
  return (_.isFinite(val)) ? Number(val) : val;
}
...
Collection.prototype.findById = function findById(id) {
  return _.find(this.models, {id: parseNumber(id)});
};


Collection.prototype.findOrCreate = function findOrCreate(id) {
  if(id === '_new') return this.createModel();
  return this.findById(parseNumber(id)) || this.createModel();
};
...
Collection.prototype.removeById = function removeById(id) {
  _.remove(this.models, {id: parseNumber(id)});
};
...
Collection.prototype.fetch = function fetch() {
  ...
  // fetch rows
  if(prom && prom.then) prom = prom.then(fetchRows);
  else prom = fetchRows();
